...
if not visited_key in global_env:
    ...
    try:
        import pyluos
        subprocess.check_call([sys.executable, "-m", "pip", "install", "pyluos", "--upgrade", "--quiet"])
...
for item in env.get("CPPDEFINES", []):
    if isinstance(item, tuple) and item[0] == "LUOSHAL":
        ...
        if (path.exists("Network/Robus/HAL/" + item[1]) and path.exists("Engine/HAL/" + item[1])):
            if not visited_key in global_env:
                print("\t*\033[0;32m %s HAL selected for Luos and Robus.\033[0m\n" % item[1])
        else:
            if not visited_key in global_env:
        ...
        env.Append(SRC_FILTER=["+<../../../Network/Robus/HAL/%s/*.c>" % item[1]])
...
for item in env.get("CPPDEFINES", []):
    ...
    if (item == 'UNIT_TEST'):
        ...
        if find_MOCK_HAL == False :
            print("\t*\033[0;32m Mock HAL for %s is selected for Luos and Robus.\033[0m\n" % current_os)
        find_MOCK_HAL = True
        find_HAL = True
        env.Replace(SRC_FILTER=sources)
