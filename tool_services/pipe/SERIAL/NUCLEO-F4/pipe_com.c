...
static void PipeCom_DMAInit(void)
{
    ...
    // Pipe to Luos
    LL_DMA_DisableStream(PIPE_RX_DMA, PIPE_RX_DMA_CHANNEL);
    LL_DMA_SetChannelSelection(PIPE_RX_DMA, PIPE_RX_DMA_CHANNEL, PIPE_TX_DMA_REQUEST);
    LL_DMA_SetDataTransferDirection(PIPE_RX_DMA, PIPE_RX_DMA_CHANNEL, LL_DMA_DIRECTION_PERIPH_TO_MEMORY);
    LL_DMA_SetStreamPriorityLevel(PIPE_RX_DMA, PIPE_RX_DMA_CHANNEL, LL_DMA_PRIORITY_LOW);
    ...
    // Prepare buffer
    LL_DMA_SetPeriphAddress(PIPE_RX_DMA, PIPE_RX_DMA_CHANNEL, (uint32_t)&PIPE_COM->DR);
    ...
    LL_DMA_EnableStream(PIPE_RX_DMA, PIPE_RX_DMA_CHANNEL);

    // Luos to Pipe
    LL_DMA_DisableStream(PIPE_TX_DMA, PIPE_TX_DMA_CHANNEL);
    LL_DMA_SetChannelSelection(PIPE_TX_DMA, PIPE_TX_DMA_CHANNEL, PIPE_TX_DMA_REQUEST);
    LL_DMA_SetDataTransferDirection(PIPE_TX_DMA, PIPE_TX_DMA_CHANNEL, LL_DMA_DIRECTION_MEMORY_TO_PERIPH);
    LL_DMA_SetStreamPriorityLevel(PIPE_TX_DMA, PIPE_TX_DMA_CHANNEL, LL_DMA_PRIORITY_LOW);
    ...
    // Prepare buffer
    LL_DMA_SetPeriphAddress(PIPE_TX_DMA, PIPE_TX_DMA_CHANNEL, (uint32_t)&PIPE_COM->DR);
...
static void PipeCom_SerialSend(void)
{
    if (is_sending == false)
    {
        ...
        LL_DMA_EnableStream(PIPE_TX_DMA, PIPE_TX_DMA_CHANNEL);
    }
}
/******************************************************************************
