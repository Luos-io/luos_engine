...
#define KERNEL_VERSION (((uint32_t)tskKERNEL_VERSION_MAJOR * 10000000UL) | ((uint32_t)tskKERNEL_VERSION_MINOR * 10000UL) | ((uint32_t)tskKERNEL_VERSION_BUILD * 1UL))
...
osMessageQueueId_t osMessageQueueNew(uint32_t msg_count, uint32_t msg_size, const osMessageQueueAttr_t *attr)
{
    ...
    if (!IS_IRQ() && (msg_count > 0U) && (msg_size > 0U))
    {
        ...
        if (attr != NULL)
        {
            if ((attr->cb_mem != NULL) && (attr->cb_size >= sizeof(StaticQueue_t)) && (attr->mq_mem != NULL) && (attr->mq_size >= (msg_count * msg_size)))
            {
                mem = 1;
            }
            else
