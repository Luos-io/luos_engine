...
void Luos_Loop(void) {
    ...
    while (MsgAlloc_LookAtLuosTask(remaining_msg_number, &oldest_ll_container) != FAILED) {
        ...
        if ((MsgAlloc_GetLuosTaskCmd(remaining_msg_number, &cmd) != SUCCEED) ||
            (MsgAlloc_GetLuosTaskSize(remaining_msg_number, &size) != SUCCEED)) {
            break;
        }
...
static error_return_t Luos_MsgHandler(container_t *container, msg_t *input) {
    ...
    switch (input->header.cmd) {
        ...
        case RTB_CMD:
            ...
            switch (input->header.size) {
                ...
                default:
                    ...
                    LUOS_ASSERT(((uint32_t) route_tab + input->header.size) <=
                                ((uint32_t) RoutingTB_Get() + (sizeof(routing_table_t) * MAX_RTB_ENTRY)));
        ...
        case WRITE_ALIAS:
            ...
            if ((((input->data[0] >= 'A') & (input->data[0] <= 'Z')) |
                 ((input->data[0] >= 'a') & (input->data[0] <= 'z'))) & (input->header.size != 0) & (wrong == false)) {
...
static void Luos_TransmitLocalRoutingTable(container_t *container, msg_t *routeTB_msg) {
    ...
    for (uint16_t i = 0; i < container_number; i++) {
        RoutingTB_ConvertContainerToRoutingTable((routing_table_t * ) & local_routing_table[entry_nb++],
                                                 &container_table[i]);
    }
    ...
    for (uint16_t i = 0; i < container_number; i++) {
        ...
        if (container_table[i].ll_container->id == DEFAULTID) {
            ...
            if (container_table[i].auto_refresh.time_ms) {
                if ((LuosHAL_GetSystick() - container_table[i].auto_refresh.last_update) >=
                    container_table[i].auto_refresh.time_ms) {
...
error_return_t Luos_ReceiveData(container_t *container, msg_t *msg, void *bin_data) {
    ...
    // Get chunk size
    uint16_t chunk_size = msg->header.size;;
...
error_return_t Luos_ReceiveStreaming(container_t *container, msg_t *msg, streaming_channel_t *stream) {
    // Get chunk size
    unsigned short chunk_size = msg->header.size;
    if (msg->header.size > MAX_DATA_MSG_SIZE) {
        chunk_size = MAX_DATA_MSG_SIZE;
    }
