 ...
 ******************************************************************************/
streaming_channel_t
Stream_CreateStreamingChannel(const void *ring_buffer, uint16_t ring_buffer_size, uint8_t data_size) {
...
uint8_t Stream_PutSample(streaming_channel_t *stream, const void *data, uint16_t size) {
    if (((size * stream->data_size) + stream->data_ptr) > stream->end_ring_buffer) {
        ...
        // check if we exceed ring buffer capacity
        LUOS_ASSERT((stream->data_ptr >= stream->sample_ptr) &&
                    ((stream->data_ptr + (size * stream->data_size)) <= stream->end_ring_buffer));
...
uint8_t Stream_GetSample(streaming_channel_t *stream, void *data, uint16_t size) {
    ...
    return nb_available_samples -= size;
}
...
uint8_t Stream_GetAvailableSampleNB(streaming_channel_t *stream) {
    ...
    if (nb_available_sample < 0) {
        ...
        nb_available_sample =
                ((stream->end_ring_buffer - stream->sample_ptr) + (stream->data_ptr - stream->ring_buffer)) /
                stream->data_size;
    }
