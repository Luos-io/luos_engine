...
static inline uint32_t MsgAlloc_BufferAvailableSpaceComputation(void) {
    ...
    if ((uint32_t) oldest_msg != 0xFFFFFFFF) {
        LUOS_ASSERT(((uint32_t) oldest_msg >= (uint32_t) & msg_buffer[0]) &&
                    ((uint32_t) oldest_msg < (uint32_t) & msg_buffer[MSG_BUFFER_SIZE]));
        // There is some tasks
        if ((uint32_t) oldest_msg > (uint32_t) data_end_estimation) {
            ...
            stack_free_space = ((uint32_t) oldest_msg - (uint32_t) & msg_buffer[0]) +
                               ((uint32_t) & msg_buffer[MSG_BUFFER_SIZE] - (uint32_t) data_end_estimation);
            LuosHAL_SetIrqState(true);
        }
    } else {
...
static inline void MsgAlloc_OldestMsgCandidate(msg_t *oldest_stack_msg_pt) {
    if ((uint32_t) oldest_stack_msg_pt > 0) {
        LUOS_ASSERT(((uint32_t) oldest_stack_msg_pt >= (uint32_t) & msg_buffer[0]) &&
                    ((uint32_t) oldest_stack_msg_pt < (uint32_t) & msg_buffer[MSG_BUFFER_SIZE]));
        ...
        if ((uint32_t) oldest_stack_msg_pt > (uint32_t) current_msg) {
            ...
            stack_delta_space = ((uint32_t) oldest_stack_msg_pt - (uint32_t) & msg_buffer[0]) +
                                ((uint32_t) & msg_buffer[MSG_BUFFER_SIZE] - (uint32_t) current_msg);
        }
        ...
        if ((uint32_t) oldest_msg > (uint32_t) current_msg) {
            ...
            // we have to decay it to be able to define delta
            oldest_msg_delta_space = ((uint32_t) oldest_msg - (uint32_t) & msg_buffer[0]) +
                                     ((uint32_t) & msg_buffer[MSG_BUFFER_SIZE] - (uint32_t) current_msg);
        }
        // Compare deltas
...
void MsgAlloc_ValidHeader(uint8_t valid, uint16_t data_size) {
    ...
    if (((uint32_t) used_msg >= (uint32_t) current_msg) &&
        ((uint32_t) used_msg <= (uint32_t)(&current_msg->data[data_size + 2]))) {
        used_msg = NULL;
...
void MsgAlloc_EndMsg(void) {
    ...
    LUOS_ASSERT(!(msg_tasks_stack_id > 0) || (((uint32_t) msg_tasks[0] >= (uint32_t) & msg_buffer[0]) &&
                                              ((uint32_t) msg_tasks[0] < (uint32_t) & msg_buffer[MSG_BUFFER_SIZE])));
...
static inline error_return_t MsgAlloc_ClearMsgSpace(void *from, void *to) {
    ...
    if (((uint32_t) oldest_msg >= (uint32_t) from) && ((uint32_t) oldest_msg <= (uint32_t) to)) {
        ...
        mem_stat->buffer_occupation_ratio = 100;
        while (((uint32_t) luos_tasks[0].msg_pt >= (uint32_t) from) &&
               ((uint32_t) luos_tasks[0].msg_pt <= (uint32_t) to) && (luos_tasks_stack_id > 0)) {
            // This message is in the space we want to use, clear the task
        ...
        while (((uint32_t) tx_tasks[0].data_pt >= (uint32_t) from) &&
               ((uint32_t) tx_tasks[0].data_pt <= (uint32_t) to) && (tx_tasks_stack_id > 0)) {
            // This message is in the space we want to use, clear the task
...
error_return_t MsgAlloc_PullMsgToInterpret(msg_t **returned_msg) {
    ...
    *returned_msg = (msg_t *) msg_tasks[0];
    LUOS_ASSERT(((uint32_t) * returned_msg >= (uint32_t) & msg_buffer[0]) &&
                ((uint32_t) * returned_msg < (uint32_t) & msg_buffer[MSG_BUFFER_SIZE]));
    MsgAlloc_ClearMsgTask();
    return SUCCEED;
}
 ...
 ******************************************************************************/
error_return_t
MsgAlloc_SetTxTask(ll_container_t *ll_container_pt, uint8_t *data, uint16_t crc, uint16_t size, uint8_t locahost,
                   uint8_t ack) {
    LUOS_ASSERT((tx_tasks_stack_id >= 0) && (tx_tasks_stack_id < MAX_MSG_NB) && ((uint32_t) data > 0) &&
                ((uint32_t) current_msg < (uint32_t) & msg_buffer[MSG_BUFFER_SIZE]) &&
                ((uint32_t) current_msg >= (uint32_t) & msg_buffer[0]));
    void *rx_msg_bkp = 0;
    void *tx_msg = 0;
    ...
    if (MsgAlloc_DoWeHaveSpace((void *) ((uint32_t) current_msg + size)) == FAILED) {
        ...
        if (MsgAlloc_CheckMsgSpace((void *) msg_buffer, (void *) ((uint32_t) msg_buffer + size + estimated_size)) ==
            FAILED) {
        ...
        if (MsgAlloc_DoWeHaveSpace((void *) ((uint32_t) tx_msg + size + estimated_size)) == FAILED) {
            ...
            if (MsgAlloc_CheckMsgSpace((void *) msg_buffer, (void *) ((uint32_t) msg_buffer + estimated_size)) ==
                FAILED) {
                LuosHAL_SetIrqState(true);
                return FAILED;
            }
            ...
            // Check space for the TX and RX message
            if (MsgAlloc_CheckMsgSpace((void *) ((uint32_t) tx_msg),
                                       (void *) ((uint32_t) tx_msg + size + estimated_size)) == FAILED) {
                LuosHAL_SetIrqState(true);
                return FAILED;
            }
    ...
    if (progression_size >= sizeof(header_t)) {
        ...
        memcpy((void *) ((uint32_t) current_msg_cpy + sizeof(header_t)),
               (void *) ((uint32_t) rx_msg_bkp + sizeof(header_t)), (progression_size - sizeof(header_t)));
    } else {
    ...
    if (ack != 0) {
        ...
        memcpy((void *) &((char *) tx_msg)[3], (void *) &data[3],
               size - 6); // 3 bytes already copied - 2 bytes CRC - 1 byte ack
