...
uint8_t *tx_data                        = 0;
const nrf_drv_timer_t timeout_instance  = NRF_DRV_TIMER_INSTANCE(1);
...
static void RobusHAL_TimerIRQHandler(void);
void RobusHAL_ComIrqHandler(nrfx_uart_event_t const *p_event,
                            void *p_context);
void RobusHAL_PinoutIRQHandler(uint16_t GPIO_Pin, nrf_gpiote_polarity_t action);
...
void RobusHAL_ComInit(uint32_t Baudrate)
{
    ...
    if (nrf_drv_uart_init(&uart_instance, &uart_config, (nrf_uart_event_handler_t)RobusHAL_ComIrqHandler) != NRF_SUCCESS)
    {
        return;
    }
...
void RobusHAL_SetTxState(uint8_t Enable)
{
    ...
    if (Enable == true)
    {
        ...
        if (TX_EN_PIN != DISABLE)
        {
             ...
             *******************************************************************/
           // nrf_gpio_pin_clear(NRF_GPIO_PIN_MAP(0, TX_EN_PIN));
            nrf_gpio_pin_set(NRF_GPIO_PIN_MAP(0, TX_EN_PIN));
            tx_en = true;
...
void RobusHAL_SetRxState(uint8_t Enable)
{
    ...
    if (Enable == true)
    {
         ...
         ************************************************************************/
       // nrf_uart_event_clear(uart_instance.uart.p_reg, NRF_UART_EVENT_RXDRDY);
        //nrf_drv_uart_rx_disable(&uart_instance);
        //nrf_uart_int_enable(uart_instance.uart.p_reg, NRF_UART_INT_MASK_RXDRDY);
       // nrf_drv_uart_rx_enable(&uart_instance);
        nrf_drv_uart_rx(&uart_instance, rx_buf, 1);
        //nrf_uart_disable(uart_instance.uart.p_reg);
        //nrf_uart_enable(uart_instance.uart.p_reg);
       
        //nrf_uart_int_enable(uart_instance.uart.p_reg, NRF_UART_INT_MASK_RXDRDY);
        //nrf_uart_int_enable(uart_instance.uart.p_reg, NRF_UART_INT_MASK_RXTO);
        //nrf_uart_int_enable(uart_instance.uart.p_reg, NRF_UART_INT_MASK_CTS);
    }
    else
    {
        /*************************************************************************
        ...
        //nrf_uart_event_clear(uart_instance.uart.p_reg, NRF_UART_EVENT_RXDRDY);
        //nrf_drv_uart_rx_disable(&uart_instance);
        //nrf_uart_int_disable(uart_instance.uart.p_reg, NRF_UART_INT_MASK_RXDRDY);
       // nrf_drv_uart_rx_enable(&uart_instance);
        //nrf_uart_enable(uart_instance.uart.p_reg);
        //nrf_uart_int_disable(uart_instance.uart.p_reg, NRF_UART_INT_MASK_RXTO);
        //nrf_uart_int_disable(uart_instance.uart.p_reg, NRF_UART_INT_MASK_CTS);
        //nrf_drv_uart_rx_disable(&uart_instance);
                            ...
                            void *p_context)
{
    ...
    if (p_event->type == NRFX_UART_EVT_RX_DONE)
    {
        ...
        uint8_t rx_data;
        rx_data = rx_buf[0];
        ctx.rx.callback(&rx_data); // Send received byte to the Robus state machine
    }
    else if (p_event->type == NRFX_UART_EVT_TX_DONE)
    {
        // Handle tx
        nrf_gpio_pin_set(NRF_GPIO_PIN_MAP(0, 28));
    ...
    else
    {
        nrf_gpio_pin_set(NRF_GPIO_PIN_MAP(0, 29));
        nrf_gpio_pin_clear(NRF_GPIO_PIN_MAP(0, 29));
        error = nrf_drv_uart_errorsrc_get(&uart_instance);
        RobusHAL_ResetTimeout(DEFAULT_TIMEOUT);
        // handle errors
...
void RobusHAL_ComTransmit(uint8_t *data, uint16_t size)
{
    ...
    //nrf_uart_int_enable(uart_instance.uart.p_reg, NRF_UART_INT_MASK_TXDRDY);
    //nrf_uart_enable(uart_instance.uart.p_reg);
    tx_en = true;
    ...
    if (size > 1)
    {
        ...
        if (size <= 128)
        {
            if (nrf_drv_uart_tx(&uart_instance, tx_data, size) == NRF_SUCCESS)
            {
                ;
            }
            //data_size_to_transmit = 0;
        ...
        {
            tx_data = data + 128;
            nrf_drv_uart_tx(&uart_instance, tx_data, 128);
    ...
    else
    {
        ...
        while (nrf_drv_timer_capture_get(&timeout_instance, NRF_TIMER_CC_CHANNEL1) < TIMEOUT_ACK)
            ;
        //RobusHAL_SetTxState(true); // Enable TX
        // Transmit the only byte we have
        nrf_drv_uart_tx(&uart_instance, data, 1);
...
uint8_t RobusHAL_GetTxLockState(void)
{
    ...
    //if (nrf_drv_uart_rx(&uart_instance, rx_buf, 1) == NRF_ERROR_BUSY)
    //{
        //RobusHAL_ResetTimeout(DEFAULT_TIMEOUT);
        //result = true;
    return result;
...
static void RobusHAL_TimeoutInit(void)
{
     ...
     ************************************************************************/
    uint32_t time_ticks;
    nrf_drv_timer_config_t timeout_config = NRF_DRV_TIMER_DEFAULT_CONFIG;
    timeout_instance.p_reg->PRESCALER = Timer_Prescaler - 1;
    if (nrf_drv_timer_init(&timeout_instance, &timeout_config, (nrfx_timer_event_handler_t)RobusHAL_TimerIRQHandler) != NRF_SUCCESS)
    {
    }
}
/******************************************************************************
...
void RobusHAL_ResetTimeout(uint16_t nbrbit)
{
    ...
    uint32_t time_ticks;
    // Disable counter
    nrf_drv_timer_disable(&timeout_instance);
    // Clear pending interrup and flag
    nrf_drv_timer_clear(&timeout_instance);
    // Reset counter
    if (nbrbit != 0)
    {
        time_ticks = nrf_drv_timer_us_to_ticks(&timeout_instance, nbrbit);

        nrf_drv_timer_extended_compare(&timeout_instance, NRF_TIMER_CC_CHANNEL2, (time_ticks), NRF_TIMER_SHORT_COMPARE2_CLEAR_MASK, true);
        // NVIC_EnableIRQ(LUOS_TIMER_IRQ);
        nrf_drv_timer_enable(&timeout_instance);
        // Reload value
        // Enable counter
    }
}
 ...
 * @return None
 ******************************************************************************/
void RobusHAL_TimerIRQHandler(void)
{
    /*************************************************************************
    ...
    if (nrf_drv_timer_is_enabled(&timeout_instance))
    {
        nrf_drv_timer_clear(&timeout_instance);
        nrf_drv_timer_disable(&timeout_instance);
        //nrf_drv_timer_enable(&timeout_instance);
...
static void RobusHAL_GPIOInit(void)
{
    ...
    while (nrf_drv_clock_hfclk_is_running() == 0)
    {
        // Do nothing.
    }
    ...
    if (RX_EN_PIN != DISABLE)
    {
        ...
        // Output
        // No pull
        //const nrfx_gpiote_out_config_t rx_config = NRFX_GPIOTE_CONFIG_OUT_TASK_HIGH;
       // nrf_drv_gpiote_out_init(RX_EN_PIN, &rx_config);
    }
    ...
    if (TX_EN_PIN != DISABLE)
    {
        ...
        // Output
        // No pull
        nrf_gpio_cfg_output(NRF_GPIO_PIN_MAP(0, TX_EN_PIN));
        nrf_gpio_pin_clear(NRF_GPIO_PIN_MAP(0, TX_EN_PIN));
    }
    ...
    for (uint8_t i = 0; i < NBR_PORT; i++)
    {
        ...
        // Pull down
        // Setup PTP lines
        const nrfx_gpiote_out_config_t ptp_config = NRFX_GPIOTE_CONFIG_OUT_TASK_LOW;
        nrf_drv_gpiote_out_init(PTP[i].Pin, &ptp_config);
...
void RobusHAL_SetPTPDefaultState(uint8_t PTPNbr)
{
     ...
     *************************************************************************/
    nrfx_gpiote_in_config_t ptp_config;
    nrf_drv_gpiote_in_event_disable(PTP[PTPNbr].Pin);
    nrf_drv_gpiote_in_uninit(NRF_GPIO_PIN_MAP(0, PTP[PTPNbr].Pin));
    nrf_drv_gpiote_out_clear(NRF_GPIO_PIN_MAP(0, PTP[PTPNbr].Pin));
    nrf_drv_gpiote_out_uninit(NRF_GPIO_PIN_MAP(0, PTP[PTPNbr].Pin));
    ptp_config.hi_accuracy     = true;
    ptp_config.is_watcher      = false;
    ptp_config.pull            = NRF_GPIO_PIN_PULLDOWN;
    ptp_config.sense           = NRF_GPIOTE_POLARITY_LOTOHI;
    ptp_config.skip_gpio_setup = false;
    if (nrf_drv_gpiote_in_init(NRF_GPIO_PIN_MAP(0, PTP[PTPNbr].Pin), &ptp_config, (nrfx_gpiote_evt_handler_t)RobusHAL_PinoutIRQHandler) != NRF_SUCCESS)
    {
        uint8_t error = 1;
    }
    nrf_drv_gpiote_in_event_enable(PTP[PTPNbr].Pin, true);
}
/******************************************************************************
...
void RobusHAL_SetPTPReverseState(uint8_t PTPNbr)
{
    ...
    ptp_config.is_watcher      = false;
    ptp_config.pull            = NRF_GPIO_PIN_PULLDOWN;
    ptp_config.sense           = NRF_GPIOTE_POLARITY_HITOLO;
    ptp_config.skip_gpio_setup = false;
    nrf_drv_gpiote_in_init(NRF_GPIO_PIN_MAP(0, PTP[PTPNbr].Pin), &ptp_config, (nrf_drv_gpiote_evt_handler_t)RobusHAL_PinoutIRQHandler);
    nrf_drv_gpiote_in_event_enable(PTP[PTPNbr].Pin, true);
}
/******************************************************************************
 * @brief Set PTP line
...
void RobusHAL_PushPTP(uint8_t PTPNbr)
{
    ...
    nrf_drv_gpiote_in_event_disable(PTP[PTPNbr].Pin);
    nrf_drv_gpiote_in_uninit(NRF_GPIO_PIN_MAP(0, PTP[PTPNbr].Pin));
    const nrfx_gpiote_out_config_t ptp_config = NRFX_GPIOTE_CONFIG_OUT_SIMPLE(1);
    nrf_drv_gpiote_out_init(NRF_GPIO_PIN_MAP(0, PTP[PTPNbr].Pin), &ptp_config);
    nrfx_gpiote_out_task_enable(NRF_GPIO_PIN_MAP(0, PTP[PTPNbr].Pin));
    nrf_drv_gpiote_out_set(NRF_GPIO_PIN_MAP(0, PTP[PTPNbr].Pin));
}
/******************************************************************************
 * @brief Get PTP line
 * @param PTP branch
...
uint8_t RobusHAL_GetPTPState(uint8_t PTPNbr)
{
     ...
     ************************************************************************/
    return (nrf_drv_gpiote_in_is_set(NRF_GPIO_PIN_MAP(0, PTP[PTPNbr].Pin))); // Value of the PTP Line
}
/******************************************************************************
...
void RobusHAL_ComputeCRC(uint8_t *data, uint8_t *crc)
{
    ...
    // CRC HW calculation
    uint16_t result_crc = crc16_compute(data, 1, crc);
    crc                 = (uint8_t *)result_crc;
#else
