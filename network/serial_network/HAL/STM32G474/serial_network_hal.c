...
void SerialHAL_Init(uint8_t *rx_buffer, uint32_t buffer_size)
{
    ...
    LPUART_InitStruct.BaudRate            = SERIAL_COM_DEFAULTBAUDRATE;
    ...
    LL_LPUART_ClearFlag_IDLE(SERIAL_COM);
    LL_LPUART_EnableIT_IDLE(SERIAL_COM);
...
void SERIAL_COM_IRQHANDLER()
{
    ...
    if (LL_LPUART_IsActiveFlag_IDLE(SERIAL_COM))
    {
        LL_LPUART_ClearFlag_IDLE(SERIAL_COM);
